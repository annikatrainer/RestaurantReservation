 ...
 * @property {number} num_seats
 * @property {number|null} confirmation_ID
 * @property {number} restaurant_ID
 * @property {number} table_num
 */
export const handler = async (event) => {
  ...
  let FinalAvailTables = (tables, restaurant_ID) => {
    return new Promise((resolve, reject) => {
  ...
  try {
    ...
    if (heep) {
     ...
     // const correctRestaurants = await CorrectRestaurants(cleanedTables, allTables)
      ...
      const finalAvailTables = await FinalAvailTables(finalTables, event.restaurant_ID)
      const cleanfinaltables = finalAvailTables.flatMap(restaurant => restaurant.map(table => table.table_ID));
      const tables5 = cleanfinaltables.filter(element => !cleanTableIDs.includes(element));
      const finalfinalTables = await FinalTables(tables5)

      if (tables5.length == 0){
        response = { statusCode: 400, error: "No availble Tables try another time period" }
      } else {
        response = {
          statusCode: 200,
          result:  finalfinalTables
        };
      }
    }
